...
from core.preprocess import preprocess, Resize, Normalize, Permute, PadStride
from core.visualize import visualize_box_mask
from core.check import enable_static_mode


# model_dir = 'models/model-mydata'
Model_DIR='models/model-mydata'
# 'models/ppyolo_drone_r18'
...
def predict_image(image_file,threshold = 0.5):
    model_dir = Model_DIR
...
def predict_save_video(video_file,threshold = 0.5):
    model_dir=Model_DIR
    config = Config(model_dir)
    detector = Detector(
        config, model_dir, use_gpu=True, run_mode='fluid')
...
def predict_video(video_file,threshold = 0.5):
    model_dir=Model_DIR
    config = Config(model_dir)
    detector = Detector(
        config, model_dir, use_gpu=True, run_mode='fluid')
...
def predict_camera(threshold=0.5):
...
def main():
    ...
    # predict from image
    # t1 = time.time()
    # image_file = '/home/yuchen/multi-drone-detect-track/demo/000050.jpg'
    # predict_image(image_file,threshold=0.8)
    # t2=time.time()
    ...
    # video_file = '/home/yuchen/multi-drone-detect-track/demo/20201102205045.mp4'#
    # t1 = time.time()
    # predict_video(video_file, threshold=0.8)
...
if __name__ == '__main__':
    ...
    # parser.add_argument(
    #     "--video_file", type=str, default='demo/20201102205045.mp4', help="Path of video file.")
    # parser.add_argument(
    #     "--camera_id",
    #     type=int,
    #     default=-1,
